...
namespace PingCastle.ADWS
{
    public abstract class ADConnection : IADConnection
    {
        ...
        protected ADConnection(IIdentityProvider identityProvider)
        {
        ...
        public static string EncodeSidToString(string sid)
        {
            try
            {
                ...
                var realsid = new SecurityIdentifier(sid);
        ...
        public virtual string ConvertSIDToName(string sidstring, out string referencedDomain)
        {
            ...
            try
            {
                if (Credential != null)
                {
                    ...
                    {
                        identity = IdentityProvider.GetWindowsIdentityForUser(Credential, Server);
                    }
                }

                if (IdentityProvider != null)
                {
                    return IdentityProvider.ConvertSIDToName(identity, sidstring, Server, out referencedDomain);
                }
