...
namespace PingCastle.Healthcheck.Rules
{
    ...
    public class HeatlcheckRuleStaledObsoleteW10 : RuleBase<HealthcheckData>
    {
        ...
        public void AnalyzeAndPopulateReplacementToDo(HealthcheckData healthcheckData, List<string> remainingDetails)
        {
            ReplacementToDo.Clear();
            int totalActive = 0;
            foreach (var detail in remainingDetails)
            {
                var match = Regex.Match(detail, @"Active:\s*(\d+)");
                if (match.Success)
                {
                    totalActive += int.Parse(match.Groups[1].Value);
                }
            }
            ReplacementToDo["{activeComputers}"] = totalActive.ToString();
        }
